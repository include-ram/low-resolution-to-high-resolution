...
# Add argument parser
parser = argparse.ArgumentParser(description='VQGAN-Transformer Super Resolution FSDP Training')
...
parser.add_argument('--debug', action='store_true', help='Enable debug mode (non-distributed)')

parser.add_argument('--use_mixed_precision', action='store_true', help='Use mixed precision for FSDP')
parser.add_argument('--cpu_offload', action='store_true', help='Use CPU offloading for FSDP')
parser.add_argument('--sharding_strategy', type=str, default='FULL_SHARD', 
                   choices=['FULL_SHARD', 'SHARD_GRAD_OP', 'NO_SHARD'], help='FSDP sharding strategy')
parser.add_argument('--min_params_for_wrap', type=int, default=100000, help='Min parameters for auto-wrapping')
...
config = {
    ...
    'min_params_for_wrap': args.min_params_for_wrap,
}

# Helper function to get FSDP sharding strategy
def get_sharding_strategy(strategy_name):
    if strategy_name == 'FULL_SHARD':
        return ShardingStrategy.FULL_SHARD
    elif strategy_name == 'SHARD_GRAD_OP':
        return ShardingStrategy.SHARD_GRAD_OP
    elif strategy_name == 'NO_SHARD':
        return ShardingStrategy.NO_SHARD
    else:
        raise ValueError(f"Unknown sharding strategy: {strategy_name}")

# Modified init_distributed function with better environment detection for FSDP
...
if is_master:
    ...
    print(f"Using {world_size} GPUs for training with FSDP")
...
# Configure FSDP mixed precision settings
...
# Define updated auto-wrap policy function
# Define custom auto-wrap policy function
# Define custom auto-wrap policy function
def min_params_wrap_policy(module, recurse=True, **kwargs):
    # Count the parameters that require gradients
    num_params = sum(p.numel() for p in module.parameters() if p.requires_grad)
    return num_params >= config['min_params_for_wrap']
auto_wrap_policy = min_params_wrap_policy
if distributed:
...
# Modified train_vqgan function with FSDP support and resume capability
def train_vqgan(vqgan, discriminator, train_sampler, dataloader, num_epochs=100, lr=2e-4, 
               checkpoint_dir=None, device='cuda', start_epoch=0):
    """Train the VQGAN model with FSDP support"""
    if is_master:
        print(f"Training VQGAN for {num_epochs} epochs starting at epoch {start_epoch}...")
    
    # Clear CUDA cache before FSDP
    torch.cuda.empty_cache()
    
    # Wrap model with FSDP if distributed
    # In the train_vqgan function, change how you wrap the VQGAN model:

# Wrap model with FSDP if distributed
    if distributed:
        # First, get a reference to the vector quantizer
        vector_quantizer = vqgan.quantize
        
        # Remove the vector quantizer from the VQGAN so it won't be wrapped
        vqgan.quantize = None
        
        # Wrap the VQGAN model with FSDP
        vqgan = FSDP(
            vqgan,
            auto_wrap_policy=min_params_wrap_policy,
        ...
        # Put the vector quantizer back 
        vqgan._fsdp_wrapped_module.quantize = vector_quantizer
        
        # Wrap discriminator with FSDP
        discriminator = FSDP(
            discriminator,
            auto_wrap_policy=min_params_wrap_policy,
            mixed_precision=mixed_precision_policy,
    ...
    if start_epoch > 0 and checkpoint_dir:
        ...
        if os.path.exists(checkpoint_path):
            ...
            # For FSDP, we need to load state dict differently
            if distributed:
                with FSDP.state_dict_type(vqgan, StateDictType.FULL_STATE_DICT):
                    vqgan_state = vqgan.state_dict()
                    vqgan.load_state_dict(checkpoint['vqgan'])
                
                with FSDP.state_dict_type(discriminator, StateDictType.FULL_STATE_DICT):
        ...
        if is_master:
            ...
            if (epoch + 1) % config['evaluate_every'] == 0 or epoch == 0:
                with torch.no_grad():
                    ...
                    if distributed:
                        # Need to consolidate the full model to visualize with FSDP
                        with FSDP.summon_full_params(vqgan):
                            recon_imgs, _, _ = vqgan(imgs_high[:4])
                    else:
                        recon_imgs, _, _ = vqgan(imgs_high[:4])
            ...
            if checkpoint_dir and ((epoch + 1) % config['save_checkpoint_every'] == 0 or epoch == num_epochs - 1):
                ...
                if distributed:
                    # Use FSDP state dict utility to get the full model
                    with FSDP.state_dict_type(vqgan, StateDictType.FULL_STATE_DICT):
                        vqgan_state = vqgan.state_dict()
    ...
    # Return unwrapped model for downstream use
    if distributed:
        # Use FSDP utility to get the consolidated model
        with FSDP.summon_full_params(vqgan, writeback=True):
            # Create a new instance to hold the unwrapped weights
            n_downsampling = 4  # Fixed parameter
            unwrapped_vqgan = FixedVQGAN(
                in_channels=3, 
            ...
            with FSDP.state_dict_type(vqgan, StateDictType.FULL_STATE_DICT):
                vqgan_state = vqgan.state_dict()
                unwrapped_vqgan.load_state_dict(vqgan_state)
                
        return unwrapped_vqgan
    else:
        return vqgan


# Modified train_transformer function with FSDP support and resume capability
def train_transformer(super_res_model, train_sampler, dataloader, num_epochs=100, lr=1e-4, 
                     checkpoint_dir=None, device='cuda', start_epoch=0):
    """Train the transformer model for super-resolution with FSDP support"""
    if is_master:
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for i, (imgs_low, imgs_high) in enumerate(dataloader):
            ...
            if distributed:
                # Get the unwrapped transformer
                with FSDP.summon_full_params(transformer, writeback=False):
                    # Create temporary model for forward
    ...
    # Return the unwrapped transformer model for downstream use
    if distributed:
        # Get the full consolidated model
        with FSDP.summon_full_params(transformer, writeback=True):
            # Create a new instance with same parameters
            unwrapped_transformer = TransformerModel(
                codebook_size=config['codebook_size'],
                seq_len=transformer.pos_embedding.shape[1],
            ...
            with FSDP.state_dict_type(transformer, StateDictType.FULL_STATE_DICT):
                transformer_state = transformer.state_dict()
                unwrapped_transformer.load_state_dict(transformer_state)
                
        # Update the super_res_model with unwrapped transformer
        super_res_model.transformer = unwrapped_transformer
    else:
...
def main():
    ...
    train_dataloader = DataLoader(
        ...
        pin_memory=True,
        drop_last=True  # Important for distributed training to have same batch size on all ranks
    )
...
if __name__ == "__main__":
    try:
        if is_master:
            print(f"Starting VQGAN-Transformer Super Resolution training with FSDP...")
        ...
        # Try to clean up distributed process group
